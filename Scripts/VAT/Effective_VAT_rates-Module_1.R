...
#weight_hbs <- read_excel("Data/VAT/Weight_hbs2020.xls")
...
# I.Business as usual -----------------------------------------------------
           ...
           # EFFECTIVE_VAT_RATES<-left_join(TAX_BASE_TOTAL,SIMULATION,by = c("PRODUCT_INDUSTRY_CODE"))%>%
            ...
            # 2. HBS Analysis ------------------------------------------------------------
                  ...
                  # 2.2 Merging with HBS Available assets and adding weights--------------------------------------------------------
            
                  # NEW TEST 
                  data4_hbs_long_weighted<-left_join(data4_hbs_long,weight_hbs,by=c("number_hh"))%>%
                    dplyr::mutate(Expenditures=Expenditures*weight)%>%
                    dplyr::select(-c(year,INTERVAL,region,type,weight))
                  
                  # sum(data4_hbs_long_test$Expenditures)
                  # [1] 825413346
                  # sum(data4_hbs_long_weighted$Expenditures)
                  # [1] 180233514476
                  
               
                  #View(data4_hbs_long_weighted)
                  # END
                  
                  
                  
                  # Merging with HBS
                  data4_hbs_long_merged<-left_join(data4_hbs_long_weighted,Revenue_VAT_TOTAL_HH_CONCORDANCE_FINAL_1,by = c("COICOP_section"="Two_digits"))%>%
...
# Testing weights new 5/1/2025
                  
                  # data4_hbs_weighted<-left_join(data4_hbs,weight_hbs,by=c("number_hh"))%>%
                  #   dplyr::select(-c(year,INTERVAL,region,type))%>%
                  #   dplyr:: mutate(across(
                  #     .cols = setdiff(names(data4_hbs_test)[sapply(data4_hbs_test, is.numeric)], "weight"),
                  #     .fns = ~ . * weight
                  #   ))%>%
                  ...
                  data4_hbs_weighted <- left_join(data4_hbs, weight_hbs, by = c("number_hh")) %>%
                    dplyr::select(-c(year, INTERVAL, region, type)) %>%
                    dplyr::mutate(across(
                      .cols = setdiff(names(.)[sapply(., is.numeric)], "weight"),
                      .fns = ~ . * weight
                    )) %>%
                    dplyr::select(-c(weight))
                 ...
                 #    
                  ...
                  FINAL_WEIGHTED_SAMPLE<-mutate(data4_hbs_weighted,
                  ...
                  data4_hbs_long_merged_deciles_vat<-left_join(data4_hbs_long_merged,FINAL_WEIGHTED_SAMPLE,by = c("number_hh"="number_hh"))%>%
                    # View(data4_hbs_long_merged_deciles) 
                    dplyr::select(number_hh,COICOP_section,VAT_REVENUES_HH,centiles,deciles,total_consumption)
                  ...
                  data4_hbs_long_merged_deciles_exp<-left_join(data4_hbs_long_merged,FINAL_WEIGHTED_SAMPLE,by = c("number_hh"="number_hh"))%>%
                    # View(data4_hbs_long_merged_deciles) 
...
# II.Simulation -----------------------------------------------------------
                  ...
                  data4_hbs_weighted$Consumption_own<-NULL
                  
                  # Preparing data for estimation of deciles and centiles groups
                  data4_hbs_weighted<-data4_hbs_weighted%>%
                    dplyr::mutate(total_consumption=`01`+`02`+`03`+`04`+`05`+`06`+`07`+`08`+`09`+`10`+`11`+`12`)
