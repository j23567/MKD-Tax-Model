      ...
      tabItem(tabName = "SettingTaxableProp_SIM",
              ...
              fluidRow(
                ...
                column(3,
                       ...
                       # Radio button 
                       radioButtons("tax_category", "Include HBS data:",
                                    choices = list("On" = "On", 
                                                   "Off" = "Off" 
                                    ),
                                    selected = "On")
                ...
                column(3,
                       ...
                       numericInput("simPreferentialRate1_CPA_SIM", "Simulation Preferential Rate 1", value = 0.05, min = 0, max = 1, step = 0.01),
...
mainServer <- function(input, output, session) {
  ...
  observeEvent(input$importExcel, {
    ...
    if (!is.null(inFile)) {
      ...
      if (!all(c("PRODUCT_INDUSTRY_CODE","PRODUCT_INDUSTRY_NAME","Current_Policy_Exempt","Current_Policy_Reduced_Rate","Current_Policy_Fully_Taxable","PreferentialVATRate_1","PreferentialVATRate_2","StandardVATRate",             
                 "ProportionExempted","ProportionPreferentialRate1","ProportionPreferentialRate2","ProportionStandardRate") %in% colnames(data))) {
        showModal(modalDialog(
          title = "Error",
          "The Excel file must contain the columns: 'PRODUCT_INDUSTRY_CODE', 'PRODUCT_INDUSTRY_NAME', 'Current_Policy_Exempt', 
                        'PRODUCT_INDUSTRY_CODE','PRODUCT_INDUSTRY_NAME','Current_Policy_Exempt','Current_Policy_Reduced_Rate','Current_Policy_Fully_Taxable','PreferentialVATRate_1','PreferentialVATRate_2','StandardVATRate'             
                        'ProportionExempted','ProportionPreferentialRate1','ProportionPreferentialRate2','ProportionStandardRate' .",
          easyClose = TRUE,
          footer = NULL
        ))
        return()
  ...
  observeEvent(input$savevatRateTableUpdate_CPA_SIM, {
    ...
    if (is.null(vatRateData_CPA_SIM) || nrow(vatRateData_CPA_SIM) == 0) {
      ...
      showModal(modalDialog(
        title = "No Updates",
        "No parameters added. Using default base data in CPA_TAXABLE_PROPORTIONS_SIM.",
        easyClose = TRUE
      ))
      return()
    }
  ...
  output$vatRateTableUpdate_CPA_SIM <- renderDT({
    ...
    colnames(table_to_display) <- c(
      ...
      "Pref. Rate 1", "Pref. Rate 2", "Standard Rate",
      "Exempted", "Pref. Prop. 1", "Pref. Prop. 2", "Standard Prop."
    )
  ...
  updateCharts <- function(chart_type = NULL) {
    ...
    if (chart_type == "VAT-GAP Metrics") {
      ...
      output$vat_structure_plt <- renderPlotly({ charts$vat_structure_plt })
      output$vat_str_plt <- renderPlotly({ charts$vat_str_plt })
      
    } else if (chart_type == "Fiscal Impact and Sectoral Breakdown of VAT Revenues") {
      ...
      output$additionalCharts <- renderUI({
        tagList(
          ...
          fluidRow(
            column(6, plotlyOutput("vat_nace_NPISH_plt", height = "400px")),
      ...
      output$vat_nace_NPISH_plt <- renderPlotly({ VAT_sectors_charts$vat_nace_NPISH_plt })
      output$vat_nace_gov_plt <- renderPlotly({ VAT_sectors_charts$vat_nace_gov_plt })
    } 
    # OD OVDE 
    else if (chart_type == "VAT Revenue Projections") {
      cat("Preparing Tax Expenditures Charts\n")
      tryCatch({
      ...
      tryCatch({
        VAT_sectors_charts <- VAT_aggregation_revenues_fun(
          forecast_combined_agg,forecast_combined_cpa_selected,
          SimulationYear
        )
      }, error = function(e) {
      ...
      output$additionalCharts <- renderUI({
        tagList(
          ...
          fluidRow(
            ...
            column(6, plotlyOutput("vat_Com_Gap_plt", height = "400px"))
          )
        )
      })
      ...
      output$vat_TE_plt <- renderPlotly({ VAT_sectors_charts$vat_TE_plt })
      output$vat_Com_Gap_plt <- renderPlotly({ VAT_sectors_charts$vat_Com_Gap_plt })
    }
    
    else if (chart_type == "Distributional Impacts on Households") {
      cat("Preparing Tax Expenditures Charts\n")
      tryCatch({
        #source("Scripts/VAT/Charts-Effective_VAT_rates.R")
        source(paste0(path1, "/Scripts/VAT/Charts-Effective_VAT_rates.R"))
      }, error = function(e) {
        cat("Error sourcing Charts-Effective_VAT_rates.R:", e$message, "\n")
        return()
      })
      output$infoBox1 <- renderInfoBox({
        Kakwani_before_reform<-p#INEQ_TOTAL_INCOME_NEW[6,1]
        infoBox(
          "Kakwani Index (Before reform)",
          paste0(Kakwani_before_reform),
      ...
      output$infoBox2 <- renderInfoBox({
        Kakwani_after_reform<-p1#INEQ_TOTAL_INCOME_NEW[17,1]
        infoBox(
          "Kakwani Index (After reform)",
          paste0(Kakwani_after_reform),
          icon = icon("chart-line"),
          color = "blue"
        )
      })
      ...
      VAT_ETR_charts <- NULL
      tryCatch({
        VAT_ETR_charts <- Effective_VAT_Rates_fun(
          vat_effective_combined,
          COICOP_DecileGroups_combined,
          COICOP_HBS_BU,
          COICOP_HBS_SIM,base_year_VAT
        )
      }, error = function(e) {
        cat("Error in Effective_VAT_Rates_fun:", e$message, "\n")
        return()
      })
      
      if (is.null(VAT_ETR_charts)) {
        cat("Error: VAT sectors charts object is NULL.\n")
        return()
      }
      ...
      output$additionalCharts <- renderUI({
        tagList(
          fluidRow(
            column(6, plotlyOutput("EffectiveVAT_Percentile_HH_plt", height = "400px")),
            column(6, plotlyOutput("COICOP_DecileGroups_plt", height = "400px"))
          ),
          fluidRow(
            column(6, plotlyOutput("COICOP_BeforeReform_plt", height = "400px")),
            column(6, plotlyOutput("COICOP_AfterReform_plt", height = "400px"))
          )
        )
      })
      
      # Render each chart with Plotly
      output$EffectiveVAT_Percentile_HH_plt <- renderPlotly({ VAT_ETR_charts$EffectiveVAT_Percentile_HH_plt })
      output$COICOP_DecileGroups_plt <- renderPlotly({ VAT_ETR_charts$COICOP_DecileGroups_plt })
      output$COICOP_BeforeReform_plt <- renderPlotly({ VAT_ETR_charts$COICOP_BeforeReform_plt })
      output$COICOP_AfterReform_plt <- renderPlotly({ VAT_ETR_charts$COICOP_AfterReform_plt })
    }
