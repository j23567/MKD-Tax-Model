suppressMessages({
  ...
  # II. SIMULATION ----
            ...
            '
                                            ...
                                            '
            #SIMULATION_EST_CAL_FACTOR = copy(SIMULATION_CALIBRATION_FACTOR)
            # NEW
           # SIMULATION_EST_CAL_FACTOR<-taxable_proportion_bu
            SIM_SCENARIO_EST_CAL_FACTOR_BU<-CPA_TAXABLE_PROPORTIONS_BU

        # new 7/12/2024
        SIM_SCENARIO_EST_CAL_FACTOR_BU <- SIM_SCENARIO_EST_CAL_FACTOR_BU %>%
          dplyr::mutate(Simulated_Policy_Exempt = ifelse(is.na(ProportionExempted), Current_Policy_Exempt, ProportionExempted),
            ...
            # # 1.1.1 Calculation of taxable proportion from COICOP----------------------------------------
            # # NEW 30/5/2024
            # 'This table is based on reading data from household expenditures.Is not in function because is need to be updated through GUI'
            # 
            ...
            # 
            # CPA_PRODUCTS_EST_TE<-CPA_PRODUCTS$Supply%>%
            #   dplyr::select(PRODUCT_INDUSTRY_CODE,Imports_CIF)%>%
            #   dplyr::group_by(PRODUCT_INDUSTRY_CODE )%>%
            #   dplyr::summarize(value=sum(Imports_CIF,na.rm = TRUE))
            # 
            # CPA_PRODUCTS_EST_TE$value[is.na(CPA_PRODUCTS_EST_TE$value)] <- 0
            # 
            # # Sum of columns for Base
            # COICOP$Base <- SUPPLY_DOM_AGGREGATE_1$value + CPA_PRODUCTS_EST_TE$value
            # 
            ...
            #SIMULATION_1 <-taxable_proportion_bu
        # new 7/12/2024
            SIM_SCENARIO_EST_TE <-CPA_TAXABLE_PROPORTIONS_BU
            ...
            # new 7/12/2024
            SIM_SCENARIO_EFFECTIVE_VAT_RATE = SIM_SCENARIO_EST_CAL_FACTOR_BU
           ...
           # SIMULATION_2$Simulation_Toggles_Reduced_Rate<-TE_EXEMPT
            ...
            CPA_PRODUCTS_BU<-CPA_PRODUCTS
            
            CPA_PRODUCTS_BU$BM_Rev <- CPA_PRODUCTS_BU$Use_K_NetPurch %>%
              dplyr::select(PRODUCT_INDUSTRY_CODE, PRODUCT_INDUSTRY_NAME, Final_consumption_expenditure_by_households) %>%
              dplyr::mutate(Final_Demand_HH = Final_consumption_expenditure_by_households*benchmark_tax_rate_bu) %>%
              dplyr::select(-Final_consumption_expenditure_by_households)
            
            
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_HH[is.na(CPA_PRODUCTS_BU$BM_Rev$Final_Demand_HH)] <- 0
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_HH[grepl("Imputed rents of owner-occupied dwellings", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
            
            
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_NPISH <- CPA_PRODUCTS_BU$Use_K_NetPurch$Final_consumption_expenditure_NPISH*benchmark_tax_rate_bu
            
            
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_NPISH[is.na(CPA_PRODUCTS_BU$BM_Rev$Final_Demand_NPISH)] <- 0
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_NPISH[grepl("Imputed rents of owner-occupied dwellings", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
            
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Government <- CPA_PRODUCTS_BU$Use_K_NetPurch$Final_consumption_expenditure_by_government*benchmark_tax_rate_bu
            
            
            
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Government[is.na(CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Government)] <- 0
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Government[grepl("Imputed rents of owner-occupied dwellings", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] <- 0
            
            
            CPA_PRODUCTS_BU$BM_Rev$Final_Demand_HH[grepl("Constructions and construction works", CPA_PRODUCTS_BU$BM_Rev$PRODUCT_INDUSTRY_NAME)] = 0
            
            
            CPA_PRODUCTS_BU$BM_Rev <- CPA_PRODUCTS_BU$BM_Rev %>%
              dplyr::mutate(Final_Demand_Total = psum(Final_Demand_HH, Final_Demand_NPISH, Final_Demand_Government))
            
            Sum_of_Final_Demand_Total = sum(CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Total)
  ...
  # 1.3 Est.Industry Share ----
            ...
            EST.IS_EST_CAL_FACTOR <- merge.data.frame(SUPPLY_DOM_EST_CAL_FACTOR, SIM_SCENARIO_EST_CAL_FACTOR_BU, key = "PRODUCT_INDUSTRY_CODE")
            ...
            EST.IS <- merge.data.frame(SUPPLY_DOM, SIM_SCENARIO_EST_CAL_FACTOR_BU, key = "PRODUCT_INDUSTRY_CODE")
  ...
  # 1.4 Est Rev ----
            ...
            CPA_PRODUCTS_EST_CAL_FACTOR_BU = copy(CPA_PRODUCTS_BU)
            ...
            EST_REV_EST_CAL_FACTOR <- USE_K_DOM_NETPURCH_EST_CAL_FACTOR %>% 
              ...
              merge.data.frame(SIM_SCENARIO_EST_CAL_FACTOR_BU, key = "PRODUCT_INDUSTRY_NAME") %>%
            ...
            CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev <- EST_REV_EST_CAL_FACTOR %>% 
            ...
            CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev <- CPA_PRODUCTS_EST_CAL_FACTOR_BU$Use_K_NetPurch %>% 
              merge.data.frame(SIM_SCENARIO_EST_CAL_FACTOR_BU, key = "PRODUCT_INDUSTRY_NAME") %>%
              ...
              merge.data.frame(CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev, key = "PRODUCT_INDUSTRY_NAME") %>%
            ...
            CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$Final_Demand_HH[CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$PRODUCT_INDUSTRY_NAME == "Constructions and construction works"] <- 
              CPA_PRODUCTS_EST_CAL_FACTOR_BU$Use_K_NetPurch$Final_consumption_expenditure_by_households[CPA_PRODUCTS_EST_CAL_FACTOR_BU$Use_K_NetPurch$PRODUCT_INDUSTRY_NAME == "Constructions and construction works"]*
              #(1-SIMULATION$Simulated_Policy_Exempt[62])*vat_rate_on_residential_construction
              # NEW 12/07/2024
              (1-SIM_SCENARIO_EST_CAL_FACTOR_BU$Simulated_Policy_Exempt[62])*vat_rate_on_residential_construction
              
              
            
            CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$Final_Demand_Total = psum(CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$Total_Revenues_from_Intermediate_Inputs, 
                                                             CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$Final_Demand_HH, 
                                                             CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$Final_Demand_NPISH, 
                                                             CPA_PRODUCTS_EST_CAL_FACTOR_BU$Est_Rev$Final_Demand_Government, na.rm = T) 
            ...
            CPA_PRODUCTS_BU$Est_Rev <- EST_REV %>% 
              dplyr::group_by(PRODUCT_INDUSTRY_CODE, PRODUCT_INDUSTRY_NAME) %>%
              dplyr::summarise(Total_Revenues_from_Intermediate_Inputs = sum(value, na.rm = T)) %>%
              dplyr::arrange(PRODUCT_INDUSTRY_CODE)
            ...
            CPA_PRODUCTS_EST_TE = copy(CPA_PRODUCTS_BU)
            ...
            CPA_PRODUCTS_EST_TE$Est_Rev <- EST_REV_EST_TE %>% 
              dplyr::group_by(PRODUCT_INDUSTRY_CODE, PRODUCT_INDUSTRY_NAME) %>%
              dplyr::summarise(Total_Revenues_from_Intermediate_Inputs = sum(value, na.rm = T)) %>%
              dplyr::arrange(PRODUCT_INDUSTRY_CODE)
            
            CPA_PRODUCTS_EST_TE$Est_Rev <- CPA_PRODUCTS_EST_TE$Use_K_NetPurch %>% 
              merge.data.frame(SIM_SCENARIO_EST_TE, key = "PRODUCT_INDUSTRY_NAME") %>%
              dplyr::mutate(Final_Demand_HH = (Simulated_Policy_Reduced_Rate*PreferentialVATRate_1+Simulated_Policy_Fully_Taxable*StandardVATRate)*
                              Final_consumption_expenditure_by_households,
                            Final_Demand_NPISH = (Simulated_Policy_Reduced_Rate*PreferentialVATRate_1+Simulated_Policy_Fully_Taxable*StandardVATRate)*
                            ...
                            Final_Demand_Government = (Simulated_Policy_Reduced_Rate*PreferentialVATRate_1+Simulated_Policy_Fully_Taxable*StandardVATRate)*
                              Final_consumption_expenditure_by_government) %>%
              merge.data.frame(CPA_PRODUCTS_EST_TE$Est_Rev, key = "PRODUCT_INDUSTRY_NAME") %>%
              dplyr::select(PRODUCT_INDUSTRY_CODE, PRODUCT_INDUSTRY_NAME, Total_Revenues_from_Intermediate_Inputs, Final_Demand_HH, Final_Demand_NPISH, Final_Demand_Government) %>%
              dplyr::arrange(PRODUCT_INDUSTRY_NAME)
            
            
            CPA_PRODUCTS_EST_TE$Est_Rev$Final_Demand_HH[CPA_PRODUCTS_EST_TE$Est_Rev$PRODUCT_INDUSTRY_NAME == "Constructions and construction works"] <- 
              CPA_PRODUCTS_EST_TE$Use_K_NetPurch$Final_consumption_expenditure_by_households[CPA_PRODUCTS_EST_TE$Use_K_NetPurch$PRODUCT_INDUSTRY_NAME == "Constructions and construction works"]*
              #(1-SIMULATION$Simulated_Policy_Exempt[62])*vat_rate_on_residential_construction
              # NEW 7/12/2024
              #(1-SIMULATION_EST_CAL_FACTOR$Simulated_Policy_Exempt[62])*vat_rate_on_residential_construction
              (1-SIM_SCENARIO_EST_TE$Simulated_Policy_Exempt[62])*vat_rate_on_residential_construction
            

            
            CPA_PRODUCTS_EST_TE$Est_Rev$Final_Demand_Total = psum(CPA_PRODUCTS_EST_TE$Est_Rev$Total_Revenues_from_Intermediate_Inputs, 
                                                             CPA_PRODUCTS_EST_TE$Est_Rev$Final_Demand_HH, 
                                                             CPA_PRODUCTS_EST_TE$Est_Rev$Final_Demand_NPISH, 
                                                             CPA_PRODUCTS_EST_TE$Est_Rev$Final_Demand_Government, na.rm = T) 
            
            # 2.4.3 Estimation of effective VAT RATE ----------------------------------------
            
            USE_K_DOM_NETPURCH_EFFECTIVE_VAT_RATE = copy(USE_K_DOM_NETPURCH)
            CPA_PRODUCTS_EFFECTIVE_VAT_RATE = copy(CPA_PRODUCTS_BU)
  ...
  # 2. SIMULATION RESULTS ----  
            ...
            '
              ...
              CPA_PRODUCTS_1->CPA_PRODUCTS_EST_TE
            ...
            # NEW
            Results_EST_CAL_FACTOR$VAT_Gap <- as.data.frame(sum(CPA_PRODUCTS_EST_CAL_FACTOR_BU$BM_Rev$Final_Demand_Total))
            ...
            Locked_Calibration_Factor_BU<-Locked_Calibration_Factor_TEST
            ...
            Results$VAT_Gap <- as.data.frame(sum(CPA_PRODUCTS_BU$BM_Rev$Final_Demand_Total))
            colnames(Results$VAT_Gap) <- "Benchmark_VAT_LCU"
            
            Results$VAT_Gap$Uncalibrated_VAT_Est.LCU <- sum(CPA_PRODUCTS_BU$Est_Rev$Final_Demand_Total, na.rm = T)
            ...
            # NEW
            Results_EST_TE$VAT_Gap <- as.data.frame(sum(CPA_PRODUCTS_EST_TE$BM_Rev$Final_Demand_Total))
            ...
            Results$VAT_Gap$Calibration_Factor <- Locked_Calibration_Factor_BU
            ...
            Results_EST_CAL_FACTOR$VAT_Gap$Calibration_Factor <- Locked_Calibration_Factor_BU
            ...
            # Results_EFFECTIVE_VAT_RATE <- as.list(c("VAT_Gap", "Simulation"))
            # names(Results_EFFECTIVE_VAT_RATE) <- c("VAT_Gap", "Simulation")
            # 
            # 
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap <- as.data.frame(sum(CPA_PRODUCTS_EFFECTIVE_VAT_RATE$BM_Rev$Final_Demand_Total))
            # colnames(Results_EFFECTIVE_VAT_RATE$VAT_Gap) <- "Benchmark_VAT_LCU"
            # 
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Uncalibrated_VAT_Est.LCU <- sum(CPA_PRODUCTS_EFFECTIVE_VAT_RATE$Est_Rev$Final_Demand_Total, na.rm = T)
            # 
            # 
            # 
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Calibrated_VAT_Est.LCU <- sum(CPA_PRODUCTS_EFFECTIVE_VAT_RATE$Est_Rev$Final_Demand_Total, na.rm = T)*Locked_Calibration_Factor_BU
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU <- sum(CPA_PRODUCTS_EFFECTIVE_VAT_RATE$Use_VAT$Total_use_at_basic_prices, na.rm = T) - sum(CPA_PRODUCTS_EFFECTIVE_VAT_RATE$Use_VAT$Exports_FOB, na.rm = T)
            # 
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Total_VAT_Gap.LCU <- Results_EFFECTIVE_VAT_RATE$VAT_Gap$Benchmark_VAT_LCU - Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Total_VAT_Gap.Prc <- Results_EFFECTIVE_VAT_RATE$VAT_Gap$Total_VAT_Gap.LCU/Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU
            # 
            # # Manual input 
            # 
            # # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Policy_Gap.LCU <- 24407  # <------- Manual input in simulation
            # 
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Policy_Gap.LCU <-Results_EST_TE$Simulation$Simulated_Change_in_Revenues.LCU  
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Policy_Gap.Prc <- Results_EFFECTIVE_VAT_RATE$VAT_Gap$Policy_Gap.LCU/Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Compliance_Gap.LCU <- Results_EFFECTIVE_VAT_RATE$VAT_Gap$Total_VAT_Gap.LCU-Results_EFFECTIVE_VAT_RATE$VAT_Gap$Policy_Gap.LCU
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Compliance_Gap.Prc <- Results_EFFECTIVE_VAT_RATE$VAT_Gap$Compliance_Gap.LCU/Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU
            # Results_EFFECTIVE_VAT_RATE$VAT_Gap$Calibration_Factor <- Locked_Calibration_Factor_BU 
            # 
            # 
            # 
            # # Final output - Change in Revenues
            # 
            # 
            # Results_EFFECTIVE_VAT_RATE$Simulation <- as.data.frame(as.integer(Results_EFFECTIVE_VAT_RATE$VAT_Gap$Calibrated_VAT_Est.LCU-Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU))
            # colnames(Results_EFFECTIVE_VAT_RATE$Simulation) <- "Simulated_Change_in_Revenues.LCU"
            # Results_EFFECTIVE_VAT_RATE$Simulation$Simulated_Change_in_Revenues.Prc <- (Results_EFFECTIVE_VAT_RATE$Simulation$Simulated_Change_in_Revenues.LCU/Results_EFFECTIVE_VAT_RATE$VAT_Gap$VAT_Control_Total.LCU)*100
            # 
            # 
            # 2.1.4 Other files -------------------------------------------------------
            ...
            # Without text column
            Revenue_VAT_TOTAL_BU[3:7]<-Revenue_VAT_TOTAL_BU[3:7]*Locked_Calibration_Factor_BU
            ...
            # Old
            #Est_Rev1<-CPA_PRODUCTS_EST_TE$Est_Rev
            # New 22/04/2024
            #Est_Rev1<-CPA_PRODUCTS_EST_TE$Est_Rev
            ...
            MainResultsVATFinal_BU<-data.frame(
                                  ...
                                  Values= c(benchmark_tax_rate_bu,
            ...
            print("Script TAX CALCULATOR-MODULE BU Done !")     
            
            print(MainResultsVATFinal_BU)
